...
;; agent will be displayed on top of the 'server', and vice versa.
...
to setup
  ...
  ;;--Server Related----------------------------------------------------------;;
  set server-boot-delay 30 ;; Server booting time is fixed at 30 seconds.
...
to go
  ...
  ;; services need to continuously be added to the service pool until all the
  ...
  ;;   3. the 'consolidation?' switch has been enabled.
...
;;
;;============================================================================;;
;; During a simulation, services' status are updated on a
;; per 'tick' basis. On each update (tick), lifetime of each service will be
;; reduced by 1. At the same time, the resource usage of each service will
;; be updated (the distribution on the resource usage is a Beta distribution).
...
;; Server-1), there is another service, Service-2, has been scheduled to run
;; on the same server (Server-1), then during the scheduling process, the
;; scheduler must ensure that the Server-1 has sufficient resources for the
;; deployment of the Service-2, more specifically, this is determined by
;; the condition:
...
;; deployment of Service-2. When Service-2 starts running on Server-1,
;; both services will update their resource usage at the same time. Thus, it
...
;; However, the total required memory, at this moment, has exceed the physical
;; memory capacity of Server-1. If this happens, the performance of the two
...
to update-services-status
  ...
  [
    ...
    ;; Update the visual movement of services during submission, scheduling
...
to update-servers-status
  ...
  [
    ...
    ;; life-time. This will be accumulated from calculations of all
    ;; types of resources.
  ...
  [
    ...
    set sys-current-active-svr-net-util (sum [net-now] of actsvr) / (sum [ops-phy] of actsvr)
  ]
end
;;
;;
;;============================================================================;;
;; Generate services and initialise their status.
;;--PARAMETERS------------------------------------------------------------------
...
to generate-client-services [ amount ]
  ...
  [
    ...
    [
      ...
      ;; match resource usage pattern of servers. If a datacentre contains
...
;;--PARAMETERS----------------------------------------------------------------;;
;; 'the-servers':
;; A list of servers belongs to the rack.
;;============================================================================;;
...
;; identify an active server, i.e., servers in 'ON' or 'READY' or 'IDLE'
...
;; The two-step process will improve overall server utilization.
;;--PARAMETERS----------------------------------------------------------------;;
;; 'the-server-set'
;; 'the-service'
;;============================================================================;;
...
;; with busy servers, i.e., the servers are in 'ON' or 'READY' mode. It
...
;;
;;============================================================================;;
;; When a service has arrived at a scheduler, the scheduler will randomly
;; select a server from its rack, as long as the server has sufficient
;; resources for the deployment of the service.
...
;; solution, however, activities in a datacentre changes overtime, finding
...
;;
;;============================================================================;;
;; This procedure is used for migration processes and all of the
;; Max-Utilization-Fit family algorithms. The objective of the algorihtm is to
;; find a suitable server for 'the-service', so that the placement of the
;; service on the server will result in a balanced resources across all
;; types (computing/memory/network), with an additional condition
;; that the residual resources of the server are minimised. As a greedy
...
;; server for 'the-service', so that the placement of the service on the server
;; will result in minimum power consumption increase. Note that each type of
;; server has its own power consumption model. The power consumption of servers
;; were modeled based on the data collected from spec.org.
;;--PARAMETERS----------------------------------------------------------------;;
;;  'the-server-set'
;;  'the-service'
;;============================================================================;;
...
;; 'ops/mem/net-cnf', with the server. The resource reservation is necessary
;; and vitally important. Since services using different deployment methods
;; move to servers at a different speed and services are generated at different
...
;; consumption or to maximise the overall resource utilization.
...
;; possible when processor architecture of hosting server and
;; targeting servers are compatible. However, in this version of the simulator,
;; migrations between heterogeneous processor architectures are allowed.
;; NOTE: migration of services shall start from over-utlised servers first,
;; then followed by under-utilised servers, due to the following reasons:
;;  1. it is more important to maintain server's performance than
;;     saving powers or maximising resource utilization;
;;  2. after migrating services from over-utilised servers, the number of
;;     under-utilised servers could be reduced, thus avoid unnecessary
;;     calculations. Note service migraitons are expensive operations,
;;     in terms of service performance degradation and large network bandwidth
;;     consumption.
;;--PARAMETERS----------------------------------------------------------------;;
;;  none
;;============================================================================;;
...
;;
;;============================================================================;;
;; Migrating services out of under-utilised servers. A constituent under-utilsed
...
;;
;;============================================================================;;
;; Migrating services out of over-utilised servers. A constituent over-utilised
;; server is a server on which any of the CPU/MEM/NET resource utilization is
...
;; this is that reserved resources are those from (1) the services is being
;; migrated to other servers, or (2) a newly scheduled service that is on its
;; way to the server.
;;--PARAMETERS----------------------------------------------------------------;;
;;  None
;;============================================================================;;
...
;;
;;============================================================================;;
;; Calculating service migration delay. This will be based on the current
...
;;
;;============================================================================;;
;; When services require more resources but the request can't be satisfied by
;; the hosting server, performance of the services will be affected. In this
;; case, all services will be penalised based on how much total excessive
...
;;
;;  'the-ext-unit':
;;  It is a normalised amounts of unit that has exceeded the available physical
...
;;  of the server - the total available resources of the server) divided by
;;  the total available resources of the server.
;;
;; 'the-res-min':
;;  This input indicates the minimum value of 'the-res-diff'.
;;============================================================================;;
...
;;
;;============================================================================;;
;; Each service is assigned to one of the schedulers, conditional on the
;; configuration set out for the datacentre.
;;
;; Scenario 1:
;; The datacentre contains heterogeneous resources/configurations, specified
;; by the global variable 'datacentre-level-heterogeneity?'; each rack
;; also has mixed types of servers specified by the global variable
;; 'rack-level-heterogeneity? = true'; and the 'resource-pattern-matching'
...
;; one type of servers specified by 'rack-level-heterogeneity? = off';
...
;; of the servers on the rack, respectively.
;;
;; Scenario 3:
...
;; be evaluated. The service assignment will then fallback to the 'closest'
;; strategy, i.e., send the service to its closest scheduler measured by the
;; distance between the service and a scheduler.
;;--PARAMETERS----------------------------------------------------------------;;
;;  'the-service':
;;  the service to be assigned to one of the available scheduelrs.
;;============================================================================;;
...
;;
;;============================================================================;;
;; Reset server status after rebooting or shutting down
;;--PARAMETERS----------------------------------------------------------------;;
;;  'the-server':
;;  a set of servers
;;============================================================================;;
...
;; ratio of different types of resources are close to each other, indicating
;; resources of different types are more evenly used.
;; The procedure is mainly used when a service is being generated, so that the
;; service knows which scheduler it needs to move toward.
;;--PARAMETERS----------------------------------------------------------------;;
;;  'the-service-ress':
;;  A list that contains the required resources of the service.
;;
;;  'the-rack-ress':
;;  A list that contains the aggregated available resources of the rack.
;;============================================================================;;
...
;;
;;============================================================================;;
;; Show model of the servers. This is useful when heterogeneous clouds
...
to print-summary
  ...
  print (word "| Average Service Lifetime (Minute)                   : " (precision ((sys-service-lifetime-total * simulation-time-unit + sys-accumulated-service-ops-sla-vio + sys-accumulated-service-mem-sla-vio + sys-accumulated-service-net-sla-vio) / total-services) 4))
  ...
  print (word "| SLA Violation due to Networking Bandwidth Shortage  : " (precision round sys-accumulated-service-net-sla-vio 4))
...
;;
;;============================================================================;;
;; Deploying servers in the datacentre. The number of servers are jointly
...
<experiments>
  ...
  <experiment name="util-homo-3stage-100" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="util-hete-3stage-100" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>(sys-current-active-servers)</metric>
    <metric>(sys-current-active-svr-ops-util)</metric>
  ...
  <experiment name="util-homo-3stage-automigr-100" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>(sys-current-active-servers)</metric>
    <metric>(sys-current-active-svr-ops-util)</metric>
  ...
  <experiment name="util-hete-3stage-automigr-100" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>(sys-current-active-servers)</metric>
    <metric>(sys-current-active-svr-ops-util)</metric>
  ...
  <experiment name="util-hete-3stage-automigr-consol-100" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>(sys-current-active-servers)</metric>
    <metric>(sys-current-active-svr-ops-util)</metric>
  ...
  <experiment name="util-homo-3stage-automigr-consol-100" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>(sys-current-active-servers)</metric>
    <metric>(sys-current-active-svr-ops-util)</metric>
  ...
  <experiment name="SBRO-PowSlaMigr-Homo-2stage-MigrConsol-100" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>(sys-power-consumption-total)</metric>
    <metric>(sys-accumulated-service-ops-sla-vio + sys-accumulated-service-mem-sla-vio + sys-accumulated-service-net-sla-vio)</metric>
    ...
    <enumeratedValueSet variable="rand-seed">
      <value value="1451801848"/>
      <value value="177404250"/>
      <value value="1663576466"/>
      <value value="2815916"/>
      <value value="1784406794"/>
      <value value="1703833258"/>
      <value value="1113423047"/>
      <value value="1442202380"/>
      <value value="109126786"/>
      <value value="1978983000"/>
      <value value="512268742"/>
      <value value="613679369"/>
      <value value="1820349073"/>
      <value value="569841347"/>
      <value value="75728347"/>
      <value value="1258897692"/>
      <value value="1518040360"/>
      <value value="1012774528"/>
      <value value="375588044"/>
      <value value="2000086374"/>
      <value value="1856845561"/>
      <value value="286986439"/>
      <value value="1217525627"/>
      <value value="2054715222"/>
      <value value="480148192"/>
      <value value="888893667"/>
      <value value="927886338"/>
      <value value="84859100"/>
      <value value="1559652005"/>
      <value value="1081093118"/>
      <value value="408216828"/>
      <value value="1111979948"/>
      <value value="616466679"/>
      <value value="1413640157"/>
      <value value="1690277660"/>
      <value value="1155137666"/>
      <value value="780410476"/>
      <value value="1958432348"/>
      <value value="1734330358"/>
      <value value="393329520"/>
      <value value="185099517"/>
      <value value="18625493"/>
      <value value="1361761091"/>
      <value value="1225028313"/>
      <value value="821977332"/>
      <value value="2094050872"/>
      <value value="1398149018"/>
      <value value="365191724"/>
      <value value="305166462"/>
      <value value="1547255696"/>
      <value value="1536075056"/>
      <value value="2145059912"/>
      <value value="1810183417"/>
      <value value="1176568401"/>
      <value value="603345716"/>
      <value value="2029905450"/>
      <value value="2028866052"/>
      <value value="483378358"/>
      <value value="1242341983"/>
      <value value="124857245"/>
      <value value="478806547"/>
      <value value="2076439526"/>
      <value value="1849973448"/>
      <value value="2121365686"/>
      <value value="580724728"/>
      <value value="1400741876"/>
      <value value="2032983"/>
      <value value="2039295349"/>
      <value value="1816341663"/>
      <value value="1787623633"/>
      <value value="562987733"/>
      <value value="621418647"/>
      <value value="2053213487"/>
      <value value="1899674599"/>
      <value value="1519813126"/>
      <value value="1199703987"/>
      <value value="629274489"/>
      <value value="877280503"/>
      <value value="1950355313"/>
      <value value="804104139"/>
      <value value="21438829"/>
      <value value="1409436658"/>
      <value value="1502997067"/>
      <value value="784419217"/>
      <value value="1736404474"/>
      <value value="1993799027"/>
      <value value="294731134"/>
      <value value="1906224651"/>
      <value value="483664263"/>
      <value value="2016123256"/>
      <value value="1667934319"/>
      <value value="129202797"/>
      <value value="223073626"/>
      <value value="1422291753"/>
      <value value="846579149"/>
      <value value="211196158"/>
      <value value="1055833638"/>
      <value value="1555709141"/>
      <value value="1971636247"/>
      <value value="1367224147"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="SBRO-PowSlaMigr-Hete-2stage-MigrConsol-100" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>(sys-power-consumption-total)</metric>
    <metric>(sys-accumulated-service-ops-sla-vio + sys-accumulated-service-mem-sla-vio + sys-accumulated-service-net-sla-vio)</metric>
